...
class WarningButton(context: Context, attributeSet: AttributeSet? = null) :
    FrameLayout(context, attributeSet) {

    private val view = inflate(context, R.layout.view_button_gray_warning, this)
    ...
    private val attributes by lazy {
        context.theme.obtainStyledAttributes(attributeSet, R.styleable.WarningButton, 0, 0)
    }
    ...
    init {
        try {
            attrText = attributes.getString(R.styleable.WarningButton_text)
            attrIcon = attributes.getDrawable(R.styleable.WarningButton_icon)
        } finally {
            attributes.recycle()
        }
